...
auto create_plugin_loader(const char * type)
{
  try {
    ...
    std::cout << "no plugins found! Did you source your workspace?" << '\n';
    std::exit(-1);
  }
}
...
void print_io_plugin_list(std::string_view name, PlugingLoader plugin_loader)
{
  ...
  std::cout << n_classes << " available " << name << ":" << '\n';
...
{
  if (argc != 3) {
    std::cout << "usage: monkey_tool show_io_plugin <plugin name>" << '\n';
    return EXIT_FAILURE;
  }
  ...
  std::cout << "value references: " << p->get_references().size() << '\n';
  print_as_list(p->get_references(), print_typed_references);

  std::cout << "perform references: " << p->get_performance_references().size() << '\n';
  print_as_list(p->get_performance_references());

  std::cout << "events: " << p->get_events().size() << '\n';
...
{
  if (argc != 3) {
    std::cout << "usage: monkey_tool show_operator <plugin name>" << '\n';
    return EXIT_FAILURE;
  }
  // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-pointer-arithmetic)
  const std::string plugin_name = argv[2];
