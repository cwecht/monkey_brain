...
bool is_assignable_to(const ValueType & source, const ValueType & target)
{
...
build_assign_action(
  const ValueType & type, std::string target, ExpressionPtr expr,
  Environment & env)
{
  ...
  if (m == TYPES_TO_MAKER_FN.end()) {
    return make_assign_action<void>(std::move(target), std::move(expr), env);
  }
  return (*m->second)(std::move(target), std::move(expr), env);
}
