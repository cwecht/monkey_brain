...
void IOPluginRegistry::add_plugin(std::unique_ptr<IOPlugin> plugin)
{
  for (const auto & [ref, type, modifier] : plugin->get_references()) {
    ...
    value_types_.emplace(ref, TypedReference{ref, type, modifier});
  }
...
std::vector<TypedReference> IOPluginRegistry::get_all_values() const
{
  std::vector<TypedReference> values;
  values.reserve(value_types_.size());
  for (const auto & [_, reference] : value_types_) {
