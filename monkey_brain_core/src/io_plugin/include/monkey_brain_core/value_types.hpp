...
bool local_contains(const Rng & rng, const T & val)
{
  return std::end(rng) != std::find(std::begin(rng), std::end(rng), val);
}
} // namespace

inline bool is_signed_integer(const ValueType & v)
{
...
inline bool is_unsigned_integer(const ValueType & v)
{
...
inline bool is_floating_point(const ValueType & v)
{
