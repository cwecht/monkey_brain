...
In order to catch syntax (and some semantic) errors in both `plugins.yaml` and `state_machine.scxml`
we can run this command:
```
monkey_tool validate scxml_state_machine ./plugins.yaml path_to_scxml:=./state_machine.scxml
```
