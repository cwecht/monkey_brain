...
public:
  ...
  : name_{std::move(name)}
    ...
    , format_string_{params["format"].IsDefined() ? params["format"].as<std::string>() : ""}
    , severity_{params["severity"].IsDefined() ? params["severity"].as<int>() : 0}
    , references_{params["params"].IsDefined() ? read_references(params["params"],
